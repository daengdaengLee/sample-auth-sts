...
- **도메인 코어(기술 비의존)**: 신뢰 판단의 결정 논리만 보유합니다. 서버 바인딩 헤더가 자신의 고유 대상값을 가리키는지, 전달할 요청이 `GetCallerIdentity` 호출 형태인지, 요청이 허용된 최대 age 안에 있는지, STS가 돌려준 ARN이 허용 신원 목록에 드는지를 판단하고 자격 발급 여부를 결정합니다. 코어는 외부 기술을 직접 알지 못하고 아래 포트 추상에만 의존합니다.
...
- **어댑터**: 포트를 실제 기술로 채우는 바깥 켜입니다. 요청을 받는 *수신 어댑터*, 허용된 STS 엔드포인트로 위임하는 *STS 신원 검증 어댑터*, 자격을 실제로 발급하는 *자격 발급 어댑터*, 시각을 제공하는 *시계 어댑터*, 실행 환경에서 설정을 읽는 *설정 어댑터* 등입니다. 이 절은 역할만 다루고 구체 구현(프로토콜/라이브러리)은 정하지 않습니다.
...
C4Component
    Container_Boundary(server, "서버(신뢰 당사자)") {
        Component(inbound, "수신 어댑터", "인바운드 어댑터", "서명된 요청을 받아 인증 유스케이스 호출")
...
1. **수신**: 수신 어댑터가 서명된 `GetCallerIdentity` 요청(서명 헤더/바디 + 서버 바인딩 헤더)을 받아 인바운드 포트로 유스케이스를 호출합니다.
2. **서버 바인딩 헤더 검증(코어)**: 바인딩 값이 *이 서버만의 고유 대상값*을 가리키는지 확인합니다. 아니면 거부합니다(혼동된 대리자 완화).
3. **전달 요청 형태 검증(코어)**: 위임할 요청이 정확히 `GetCallerIdentity` 호출인지(메서드/바디) 확인합니다. 임의 요청을 대신 내보내는 통로가 되지 않도록 합니다.
...
6. **STS 위임(신원 검증 아웃바운드 포트)**: 받은 요청을 *그대로* STS에 전달하고 호출자 신원(ARN 등)을 돌려받습니다(`인증 흐름`의 3~5번).
...
- **서버 바인딩 헤더 검증(코어)**: 코어가 서명 범위 안의 바인딩 헤더를 확인합니다. 이때 각 서버는 *자신만 받아들이는 고유한 값*을 검증해야 합니다. 여러 서버가 같은 값을 받아들이면 그 사이에서는 혼동된 대리자가 다시 가능해집니다(`개요 > 보안 고려사항`의 단서 참고).
- **전달 요청 검증(코어)**: 코어가 위임 전에 메서드/바디/액션이 `GetCallerIdentity`인지 확인해, 신원 조회가 아닌 다른 요청을 대신 내보내는 통로가 되지 않도록 합니다.
